...
def fetch_paragraphs(batch_size=100):
    """
    Fetch paragraphs from drcd_paragraphs
    """
    offset = 0
    all_paragraphs = []
    while True:
        ...
        response = supabase.table('drcd_paragraphs').select('id', 'content', 'embedding')\
...
def fetch_questions(batch_size=100):
    """
    Fetch from drcd_questions
    """
    offset = 0
    all_questions = []
    while True:
        response = supabase.table('drcd_questions').select('id', 'content').range(offset, offset + batch_size - 1).execute()
...
def process_and_update_results(questions_df, bm25, paragraphs_df):
    """
    Process with bm25 model and update to Supabase [similar_doc_ids_bm25_2].
    """
    for ind, row in questions_df.iterrows():
        ...
        try:
            ...
            # 嘗試更新 Supabase
            response = supabase.table('drcd_questions').update({
                'similar_doc_ids_bm25_2': returned_doc_ids
