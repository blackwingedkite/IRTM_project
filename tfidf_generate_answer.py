...
def fetch_paragraphs(ids):
    """
    根據給定的 ID 從 Supabase 爬取對應段落。
    :param ids: 從 Supabase 獲取的 similardocs_ids 列表
    :return: 返回包含指定 ID 段落的 DataFrame。
    """
...
def fetch_questions(batch_size=100):
    ...
    while True:
        response = supabase.table('multidoc_questions').select('id', 'question', 'similar_doc_ids_tfidf').range(offset, offset + batch_size - 1).execute()
...
def process_supabase_rag(questions, start_id=0):
    ...
    for question in questions:
        ...
        similar_doc_ids_tfidf = question['similar_doc_ids_tfidf']

        if not similar_doc_ids_tfidf:
            print(f"問題 {question_id} 沒有相關的文章，跳過...")
            continue

        # 提取文章
        retrieved_paragraphs = fetch_paragraphs(similar_doc_ids_tfidf)
        ...
        try:
            supabase.table('multidoc_questions').update({
                'gpt_answer_tfidf': generated_answer
...
def process_supabase_rag_batched(questions, batch_size=10, start_id=0):
    ...
    for i in range(0, total_questions, batch_size):
        ...
        # 批量提取相關段落 ID
        all_doc_ids = set(
            doc_id for q in batch if q.get('similar_doc_ids_tfidf') 
            for doc_id in q['similar_doc_ids_tfidf']
        )
        ...
        for question in batch:
            doc_ids = question['similar_doc_ids_tfidf']
            if not doc_ids:
        ...
        # 準備更新的資料，排除 `id` 欄位
        updates = [
            {'gpt_answer_tfidf': answers.get(question['id'], None)}
            for question in batch
        ]
